...
#----------------------------------------------------------------------------------------------------
    def show(self):
        ...
        for i in os.listdir(os.path.join(os.path.dirname(__file__),'bill')):
            if i.split('.')[-1]=='txt':
    ...
    def get_data(self,ev):
        ...
        fp=open(os.path.join(os.path.dirname(__file__),'bill',file_name),'r')
        for i in fp:
    ...
    def search(self):
        if self.var_invoice.get()=="":
            ...
            if self.var_invoice.get() in self.blll_list:
                fp=open(os.path.join(os.path.dirname(__file__),'bill',self.var_invoice.get()+'.txt'),'r')
                self.bill_area.delete('1.0',END)
                for i in fp:
                    self.bill_area.insert(END,i)
                fp.close()
